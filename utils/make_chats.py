...
from claude_indexer.chat.parser import ChatParser, ChatConversation
...
def format_chat_entry(conv: ChatConversation, compact: bool = False) -> str:
    ...
    if compact:
        ...
        lines.append(f"  - **Start Time**: {conv.metadata.start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append(f"  - **End Time**: {conv.metadata.end_time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        if conv.metadata.has_code:
            lines.append(f"  - **Has Code**: Yes")
            if conv.metadata.primary_language:
                lines.append(f"  - **Primary Language**: {conv.metadata.primary_language}")
...
def main():
    parser = argparse.ArgumentParser(description="Generate list of all Claude Code chats")
    parser.add_argument('--output', '-o', type=str, default='chats.md',
    ...
    parser.add_argument('--limit', type=int, default=None,
                       help='Limit number of chats per project')
    ...
    for project_dir in project_dirs:
        ...
        conversations = []
        try:
            for chat_file in sorted(chat_files, key=lambda p: p.stat().st_mtime, reverse=True):
