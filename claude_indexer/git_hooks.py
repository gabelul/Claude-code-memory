...
class GitHooksManager:
    ...
    def is_hook_installed(self) -> bool:
        ...
        try:
            with open(self.pre_commit_path, 'r') as f:
                content = f.read()
            return 'Claude Code Memory' in content and self.collection_name in content
        except Exception:
            return False
    
    def install_pre_commit_hook(self, indexer_path: Optional[str] = None, 
        ...
        try:
            ...
            if not indexer_path:
                ...
                for cmd in ['claude-indexer', 'python -m claude_indexer', './indexer.py']:
    ...
    def uninstall_pre_commit_hook(self, quiet: bool = False) -> bool:
        ...
        try:
            ...
            if 'Claude Code Memory' not in content:
                if not quiet:
                    print("ℹ️  Pre-commit hook exists but is not managed by Claude Code Memory")
                return True
            
            # Check if hook is only ours
            lines = content.split('\n')
            non_memory_lines = [line for line in lines 
            ...
            if len([line for line in non_memory_lines if line.strip()]) <= 2:  # Just shebang and exit
                # Remove entire hook
                ...
                if backup_path.exists():
                    ...
                    if not quiet:
                        print("✅ Removed Claude Code Memory section from pre-commit hook")
        ...
        if self.pre_commit_path.exists():
            try:
                ...
                status["hook_installed"] = (
                    'Claude Code Memory' in content and 
                    self.collection_name in content
                )
